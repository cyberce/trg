		...
		if (model.get("serviceProductList") instanceof ServiceProductList) {
			...
			if (serviceProductList != null) {
				...
				for (ServiceProduct serviceProduct : serviceProductList.getServiceProducts()) {
					item = new Item();
					item.setAuthor("TEI of Athens Swaps Service");
		...
		if (model.get("serviceNotificationList") instanceof ServiceNotificationList) {
			...
			if (serviceNotificationList != null) {
				...
				for (ServiceNotification serviceNotification : serviceNotificationList.getServiceNotifications()) {
					...
					if (serviceNotification.getListingCode() != null)
						item.setLink(emailProperties.getDomain() + "previewEvent?code=" + serviceNotification.getListingCode());
					else
