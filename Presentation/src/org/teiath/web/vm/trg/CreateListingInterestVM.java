		...
		Messagebox.show(Labels.getLabel("common.messages.cancelQuestion"),
			...
			public void onEvent(Event evt) {
				switch ((Integer) evt.getData()) {
					case Messagebox.YES:
						if (ZKSession.getAttribute("fromView") != null) {
							ZKSession.setAttribute("listingId", listing.getId());
							ZKSession.removeAttribute("fromView");
							ZKSession.removeAttribute("fromSearch");
							ZKSession.sendRedirect(PageURL.SEARCH_LISTING_VIEW);
						}
						else if (ZKSession.getAttribute("fromSearch") != null) {
							ZKSession.removeAttribute("fromView");
							ZKSession.removeAttribute("fromSearch");
							ZKSession.sendRedirect(PageURL.SEARCH_LISTING);
						}
