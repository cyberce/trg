...
public class CreateListingVM
	...
	@Wire("#transactionTypesCombo")
	private Combobox transactionTypesCombo;
	...
	public void onSave() {
		if (selectedCategory != null) {
			if (selectedCategory.getId() != -1) {
				...
				try {
					if((listing.getTransactionType().getId() == 2) && (tradeableGoods.isEmpty()))  {
						Messagebox.show(MessageBuilder.buildErrorMessage("Παρακαλώ ορίστε τουλάχιστον ένα επιθυμητό αγαθό για ανταλλαγή", Labels.getLabel("listing")),
						...
						createListingService.sendNotifications(listing);
				...
				Messagebox.show(MessageBuilder.buildErrorMessage("Παρακαλώ συμπληρώστε τα πεδία τύπος συναλλαγής και κατηγορία προϊόντος", Labels.getLabel("listing")),
						Labels.getLabel("common.messages.save_title"), Messagebox.OK, Messagebox.ERROR);
			}
		}
		else {
	...
	public void togglePrice() {
		TransactionType selectedTransactionType =  (TransactionType) transactionTypesCombo.getSelectedItem().getValue();
